...
const sendAttachments = TryCatch(async (req,res,next) => {
    ...
    if(files.length > 5) return next(new ErrorHandler("File can't be more than 5",400));
...
const renameGroup = TryCatch(async (req,res,next) => {
    ...
    if(chat.creator.toString()!==req.user.toString()) 
        return next(new ErrorHandler("You are not allowed to rename group name",403));
...
export { deleteChat,renameGroup,getChatDetails,newGroupChat, getMyChats, getMyGroups, addMembers, removeMember,leaveGroup, sendAttachments, getMessages };